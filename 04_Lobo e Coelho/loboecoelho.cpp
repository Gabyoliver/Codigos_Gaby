...
int main() {
  sf::Texture wolf_tex { loadTexture("lobo.jpeg") };
  sf::Texture rabbit_tex { loadTexture("coelho.jpeg") };
  sf::Texture grass_tex { loadTexture("grama.jpeg") };

  const int STEP {100};

  Board board (7, 5, STEP, grass_tex);
  Entity wolf(1, 1, STEP, wolf_tex);
  Entity rabbit(4, 2, STEP, rabbit_tex);
  ...
  while (window.isOpen()) {
    ...
    while (window.pollEvent(event)) {
      if (event.type == sf::Event::Closed) {
        ...
        moveEntity(event.key.code, wolf, {
        ...
        moveEntity(event.key.code, rabbit, {
          sf::Keyboard::Key::A, 
          sf::Keyboard::Key::W, 
          sf::Keyboard::Key::D, 
          sf::Keyboard::Key::S 
